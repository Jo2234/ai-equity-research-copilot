...
function mockApi(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
  ...
  if (url.includes("/companies/") && !init?.method) {
    ...
    return json({
      documents: demoDocuments.filter((document) => document.company_id === companyId)
    });
  }
  const body = JSON.parse(String(init?.body ?? "{}"));
  if (url.endsWith("/research/chat"))
    return json(buildDemoChatResponse(body.question, ["cmp-nvda", "cmp-msft"]));
  if (url.endsWith("/research/memo"))
    return json(buildDemoMemo(demoCompanies.find((company) => company.id === body.company_id)!));
  if (url.endsWith("/research/compare"))
    return json({
      question: body.question,
      comparisons: demoCompanies
        .filter((company) => body.company_ids.includes(company.id))
        .map((company) => ({
  ...
  return Promise.resolve(
    new Response(JSON.stringify({ detail: "Mock route not configured" }), {
      status: 404
    })
  );
}
...
describe("AI Equity Research Copilot workstation", () => {
  it("renders companies and document readiness states", async () => {
    ...
    expect(await screen.findByText("NVIDIA Corporation", { selector: "strong" })).toBeInTheDocument();
  ...
  it("filters the local company list from the sidebar search", async () => {
    ...
    await screen.findByText("NVIDIA Corporation", { selector: "strong" });
    await user.type(screen.getByLabelText("Search companies"), "msft");

    expect(screen.getByText("Microsoft Corporation")).toBeInTheDocument();
    expect(screen.queryByText("NVIDIA Corporation", { selector: "strong" })).not.toBeInTheDocument();
  });
  ...
  it("validates required upload form fields", async () => {
    ...
    await screen.findByText("NVIDIA Corporation", { selector: "strong" });
    await user.click(screen.getByText("Upload document", { selector: "summary" }));
  ...
  it("shows failed document parse errors for the selected company", async () => {
    ...
    expect(
      await screen.findByText(
        "The uploaded text file appears to be empty. Upload a non-empty PDF or transcript."
      )
    ).toBeInTheDocument();
  });
  ...
  vi.stubGlobal(
    ...
    vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
      if (String(input).endsWith("/documents") && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify({ detail: "Upload exceeds the size limit" }), { status: 413 })
        );
      }
  ...
  vi.stubGlobal(
    ...
    vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
      if (String(input).endsWith("/research/chat")) {
        return Promise.resolve(
          new Response(JSON.stringify({ detail: "Research service unavailable" }), { status: 503 })
        );
      }
      return mockApi(input, init);
    })
...
it("uses a preset and opens source evidence only when requested", async () => {
  const user = userEvent.setup();
  render(<App />);
  await screen.findByText("NVIDIA Corporation", { selector: "strong" });
  await user.click(screen.getByRole("button", { name: /Revenue drivers/i }));
  ...
  await user.click(screen.getByRole("button", { name: /^ask$/i }));
  await screen.findByText(/accelerated Data Center demand/i);
  expect(screen.queryByRole("dialog")).not.toBeInTheDocument();
  await user.click(screen.getAllByRole("button", { name: /FY2025 Form 10-K/i })[0]);
  expect(screen.getByRole("dialog", { name: "Source evidence" })).toBeInTheDocument();
  await user.click(screen.getByRole("button", { name: "Close source evidence" }));
  expect(screen.queryByRole("dialog")).not.toBeInTheDocument();
  await user.click(screen.getByText("Ask another question", { selector: "summary" }));
